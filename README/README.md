...
Torilo Shop now includes a complete user authentication system that allows visitors to create accounts, log in, and access protected features. The system uses Django's built-in authentication framework with custom registration forms and secure login/logout functionality. Staff users have additional permissions to manage products and categories through the admin interface.
...
- **Login**: Secure login using Django's built-in authentication views
- **Logout**: Proper logout functionality with POST method for security
- **Protected Routes**: Product and category management pages require login
- **Staff Permissions**: Staff users can add/edit/delete products and categories
- **Dashboard**: Personalized dashboard showing user information and account type
...
### 6. Navbar Updates
- **Authentication Links**: Login/Register links for unauthenticated users
- **User Greeting**: Shows "Hello, username" for logged-in users
- **Dashboard Access**: Direct link to user dashboard
- **Logout Button**: Styled logout button that matches nav design
- **Responsive Design**: Proper Bootstrap navbar structure for mobile
...
- **register**: User registration with form validation and auto-login
- **dashboard**: User dashboard showing account information and type
- **product_list**: Displays all products in a responsive card grid with stock badges
- **add_product**: Add new product form (login required, staff only)
- **edit_product**: Edit existing product form (login required, staff only)
- **delete_product**: Delete product confirmation page (login required, staff only)
...
- **add_category**: Add new category form (login required, staff only)
- **edit_category**: Edit existing category form (login required, staff only)
- **delete_category**: Delete category confirmation page (login required, staff only)
...
| `/accounts/register/` | register() | User registration page |
| `/accounts/logout/` | LogoutView | User logout |
| `/dashboard/` | dashboard() | User dashboard with account info |
| `/products/` | product_list() | Products listing with stock badges |
| `/products/add/` | add_product() | Add new product (staff only) |
| `/products/<int:pk>/` | product_detail() | Individual product details |
| `/products/<int:pk>/edit/` | edit_product() | Edit product (staff only) |
| `/products/<int:pk>/delete/` | delete_product() | Delete product (staff only) |
| `/categories/` | category_list() | Category overview with product counts |
| `/categories/add/` | add_category() | Add new category (staff only) |
| `/categories/<slug>/` | category_products() | Products filtered by category |
| `/categories/<slug>/edit/` | edit_category() | Edit category (staff only) |
| `/categories/<slug>/delete/` | delete_category() | Delete category (staff only) |
| `/about/` | about() | About page with company information |

### Apps Registered
- `accounts` - User authentication app with login, registration, and dashboard
...
### Product Detail Page
![Product Detail Page](../screenshots/02_product_page.png)
...
- Registered apps: `'accounts'`, `'products'`, and `'users'`
- DEBUG mode enabled for development
- Templates configured with APP_DIRS for app-level templates

### accounts/views.py
Contains authentication view functions:
- `register()` - Handles user registration with form validation
- `dashboard()` - Shows user account information and type

### accounts/forms.py
- `RegistrationForm` - Extends UserCreationForm with email field

### accounts/urls.py
- Login/logout routes using Django's built-in auth views
- Custom registration and dashboard routes
